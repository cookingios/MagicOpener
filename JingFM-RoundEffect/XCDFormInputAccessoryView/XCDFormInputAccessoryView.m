...
- (id) initWithResponders:(NSArray *)responders
{
	...
	UISegmentedControl *segmentedControl = [[UISegmentedControl alloc] initWithItems:@[ UIKitLocalizedString(@"前一项"), UIKitLocalizedString(@"下一项") ]];
...
- (void) setHasDoneButton:(BOOL)hasDoneButton animated:(BOOL)animated
{
    ...
    NSArray *items;
	if (hasDoneButton)
		items = [_toolbar.items arrayByAddingObject:[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemStop target:self action:@selector(done)]];
	else
